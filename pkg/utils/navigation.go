...
func RunMainLoop() {
	...
	showErrAndContinue := func(err error) {
		if err != nil {
			...
			terminal.ClearScreen()
			fmt.Printf("%s%s\n\n%s%s", theme.Accent, err, terminal.ResetFmt, "press space to continue")
	...
	refresh := func() {
		...
		cache.Clear()
		startTicker(&ticker)
		currentPath, entries = scanDirectory(currentPath)
		...
		if mode == scanMode && len(entries) > 0 {
			...
			if en.IsDir && pathExists(currentPath, en.Name) {
				...
				if cachedEntries, ok := cache.Get(p); ok {
					...
					startTicker(&ticker)
					currentPath, entries = scanDirectory(p)
					sortMode = 0
					...
					if mode == scanMode {
						if currentPath != rootPath {
							...
							if cachedEntries, ok := cache.Get(path); ok {
								...
								startTicker(&ticker)
								currentPath, entries = scanDirectory(path)
